...
class ServerForm(FlaskForm):
    ...
    ip = StringField('IP地址', validators=[DataRequired(message='不能为空'),Length(0,64, message="长度不正确")])
    port = StringField('端口号', validators=[DataRequired(message='不能为空'),Length(0,64, message="长度不正确")])
    user = StringField('用户名', validators=[DataRequired(message='不能为空'),Length(0,64, message="长度不正确")])
